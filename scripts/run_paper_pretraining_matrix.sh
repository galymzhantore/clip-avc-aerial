...
COMMON_ENV=(
  ...
  # Paper protocol: train CLIP-AVC with L_r + lambda L_c only.
  # Set CLASSIFIER_HEAD=1 for CE-assisted diagnostic runs.
  CLASSIFIER_HEAD="${CLASSIFIER_HEAD:-0}"
  CLASSIFIER_MODE="${CLASSIFIER_MODE:-context}"
  CLASSIFIER_FEATURE="${CLASSIFIER_FEATURE:-refined}"
  REFINED_TEXT_POOLING="${REFINED_TEXT_POOLING:-mean}"
